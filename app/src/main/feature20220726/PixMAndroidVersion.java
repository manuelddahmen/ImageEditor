...
public class PixMAndroidVersion extends MBitmap {
    ...
    public static PixMAndroidVersion getPixM(Image image, double maxRes) {
        return getPixM(image, (int) maxRes);
    }

    public static PixMAndroidVersion getPixM(Image image, int maxRes) {
        double f = 1.0;
        if (maxRes <= 0) {
            f = 1.0;
        ...
        PixMAndroidVersion pixM = new PixMAndroidVersion((int) (columns2), ((int) lines2));
    ...
    public PixMAndroidVersion applyFilter(FilterPixM filter) {
        PixMAndroidVersion c = new PixMAndroidVersion(columns, lines);
        double sum;
...
*/
    ...
    */
    ...
    public PixMAndroidVersion pasteSubImage(int x, int y, int w, int h) {
        PixMAndroidVersion p2 = new PixMAndroidVersion(w, h);
        for (int i = x; i < x + w; i++)
            for (int j = y; j < y + h; j++)
                for (int c = 0; c < getCompCount(); c++) {
                    setCompNo(c);
