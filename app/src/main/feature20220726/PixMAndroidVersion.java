...
public class matrix.PixMAndroidVersion extends MBitmap {
    ...
    public matrix.PixMAndroidVersion(int l, int c) {

        super(l, c);
    }

    public matrix.PixMAndroidVersion(Image image) {
    ...
    public matrix.PixMAndroidVersion(Image image, boolean isBitmap) {
        super(image);
        /*
        float[] colorComponents = new float[4];
        for (int i = 0; i < image.getWidth(); i++) {
    ...
    public matrix.PixMAndroidVersion(double[][] distances) {
    ...
    public static matrix.PixMAndroidVersion getPixM(Image image, int maxRes) {
        ...
        matrix.PixMAndroidVersion matrix.PixM = new matrix.PixMAndroidVersion((int) (columns2), ((int) lines2));
...
*/
    ...
    */
    ...
    public matrix.PixMAndroidVersion pasteSubImage(int x, int y, int w, int h) {
        matrix.PixMAndroidVersion p2 = new matrix.PixMAndroidVersion(w, h);
        for (int i = x; i < x + w; i++)
            for (int j = y; j < y + h; j++)
                for (int c = 0; c < getCompCount(); c++) {
                    setCompNo(c);
    ...
    public matrix.PixMAndroidVersion copySubImage(int x, int y, int w, int h) {
        if (w <= 0 || h <= 0)
            return null;
        matrix.PixMAndroidVersion p2 = new matrix.PixMAndroidVersion(w, h);
        for (int i = x; i <= x + w; i++)
