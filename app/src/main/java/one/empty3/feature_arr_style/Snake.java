...
public class Snake extends ProcessFile {
        ...
        for (int i = 0; i < pix.getColumns(); i++) {
            ...
            for (int j = 0; j < pix.getLines(); j++) {
                ...
                if (pOut) {
                    ...
                    out.add(GMatrix.getVector(1, new double[][]{pix.getValues(i, j), pix2.getValues(i, j)}));
                    ...
                    in.add(GMatrix.getVector(0, new double[][]{pix.getValues(i, j), pix2.getValues(i, j)}));
    ...
    public boolean process(File in, File out) {
        try {
            pix = PixM.getPixM(Objects.requireNonNull(ImageIO.read(in)).bitmap, 500);
