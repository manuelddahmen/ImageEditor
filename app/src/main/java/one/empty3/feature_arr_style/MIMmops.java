...
public class MIMmops {
    public static PixM applyMultipleFilters(PixM pixM, int level, FilterPixGMatrix... filter) {
        ...
        for (int i = 0; i < level; i++) {
            ...
            Arrays.stream(filter).sequential().forEach(new Consumer<FilterPixGMatrix>() {
                @Override
                public void accept(FilterPixGMatrix filterPixM) {
    ...
    public GMatrix matGrad(PixM image, M3 gradientX, M3 gradientY) {
        GMatrix matGrad = null;
        // image :  smoothes
     ...
     * λ1 + λ2
     */
    public void cornerStrength(GMatrix matGrad) {
