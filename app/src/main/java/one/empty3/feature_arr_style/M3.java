...
public class M3 {
    ...
    public M3(Bitmap image, int columns, int lines, int columnsIn, int linesIn) {
        this(columns, lines, columnsIn, linesIn);
        float[] colorComponents = new float[getCompCount()];
        for (int i = 0; i < columns; i++) {
            for (int j = 0; j < lines; j++) {
                int rgb = image.getPixel((int) (1.0 * i / columns * image.getWidth()), (int) (1.0 * j / lines * image.getHeight()));
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    @Deprecated
    public M3(Bitmap image, int columnsIn, int linesIn) {
    ...
    public M3 filter(FilterPixGMatrix filter1, int ii, int ij) {
