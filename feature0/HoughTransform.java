...
public class HoughTransform extends ProcessFile {
     ...
     */
    public one.empty3.libs.Color getHoughArrayImage() {
        int max = getHighestValue();
        Image image = new one.empty3.libs.Color(maxTheta, doubleHeight, one.empty3.libs.Color.TYPE_INT_RGB);
        for (int t = 0; t < maxTheta; t++) {
            for (int r = 0; r < doubleHeight; r++) {
                double value = 255 * ((double) houghArray[t][r]) / max;
                int v = 255 - (int) value;
                int c = new Color(v, v, v).toArgb();
                image.setRGB(t, r, c);
            }
        }
        return image;
